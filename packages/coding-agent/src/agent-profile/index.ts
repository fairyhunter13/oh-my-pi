...
//
// Ported from ccw's ccw-agent-profile.js (claude-code-workflows, internal/policy/omp.go). The
// mechanism lives here now; ccw keeps only the built-in profile DATA, generated to
// agent-profiles.builtin.yml. See BUILTIN_FILE below for its shape and reload rule.
//
...
// The built-in profiles and the default name, generated by ccw. Shape:
// { defaultProfile: <name>, descriptions?: { <name>: <text> }, profiles: { <name>: { <agent>: row } } }.
// Missing file or missing defaultProfile means no built-in profiles and no default applied --
// this extension then behaves as today's off, never a refusal.
const BUILTIN_FILE = join(AGENT_DIR, "agent-profiles.builtin.yml");
...
// Re-read only when BUILTIN_FILE moves, the same reload rule as the hand file below. Missing or
// unparseable means no built-in profiles and no default: apply() then behaves as off, and
// before_subagent_spawn never refuses because APPLIED_PROFILE stays null. Built-ins carry no
// account: an account identity is machine-local and must never ship as a fleet constant, so
// normalizeEntry's own account validation is the only thing that would ever admit one, and ccw
// never writes one.
const loadBuiltin = (): BuiltinState => {
	let mtimeMs: number;
	try {
		mtimeMs = statSync(BUILTIN_FILE).mtimeMs;
	} catch {
		builtinCache = EMPTY_BUILTIN;
		return builtinCache;
	}
	if (mtimeMs === builtinCache.mtimeMs) {
		return builtinCache;
	}
	try {
		const parsed = (Bun.YAML.parse(readFileSync(BUILTIN_FILE, "utf8")) || {}) as Record<string, any>;
